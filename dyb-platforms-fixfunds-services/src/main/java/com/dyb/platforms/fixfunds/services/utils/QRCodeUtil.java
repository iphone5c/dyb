...
public class QRCodeUtil {
    ...
    private static BufferedImage createImage(String content, String imgPath,
        ...
        if (DybUtils.isEmptyOrNull(imgPath)) {
            return image;
        }
        // 插入图片
    ...
    private static void insertImage(BufferedImage source, String imgPath,
        ...
        String path = null;
        if(DybUtils.isEmptyOrNull(imgPath)){
            path = ContextLoader.getCurrentWebApplicationContext().getServletContext().getRealPath("/upload");
        }else {
            path = ContextLoader.getCurrentWebApplicationContext().getServletContext().getRealPath(imgPath);
        }
    ...
    public static String encode(String content, String imgPath, String destPath,
                              boolean needCompress) throws Exception {
        String path = null;
        if(DybUtils.isEmptyOrNull(destPath)){
            path = ContextLoader.getCurrentWebApplicationContext().getServletContext().getRealPath("/upload");
        }else {
            path = ContextLoader.getCurrentWebApplicationContext().getServletContext().getRealPath(destPath);
        }
